...
// use initialize function args here
    function setRandomWords(uint256 _requestId, uint256 index, bytes memory _data) public onlyOwner returns (uint256 requestId) {
        require(s_requests[_requestId].exists, "request not found");
        (bytes memory _randomWords, bytes memory _signature, address signer) = abi.decode(_data,(bytes,bytes,address));
