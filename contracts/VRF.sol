...
contract VRF {
    ...
    function getMessageHash(bytes memory _randomWords) private pure returns (bytes32) {
    ...
    function getRequestStatus(uint256 _requestId) public view returns (bool fulfilled, bytes memory randomWords) {
        require(s_requests[_requestId].exists, "request not found");
        RequestStatus memory request = s_requests[_requestId];
